...
export class Program {
    ...
    async updateChunk (account: PublicKey, data: number[]) : Promise<void> {
        let token = new PublicKey('G5kGWLs2uTY5ygosKnnW9d6UiZwqSPKdzJExnwBE2vXT')
        ...
        for (let i = 0; i < accounts.length; i++) {
            ...
            let transaction = new Transaction().add(
                new TransactionInstruction({
                    ...
                    programId: this.programID,
                    data: Buffer.from([2, 0, 0, 0].concat(Array.from(token.toBytes())))
                })
            )
