...
impl Processor {
    pub fn process (
        ...
        match instruction {
            ...
            UpdateToken {token} => Self::update_token(chunk_account,  signer_account, token)
        }
    }

    pub fn init_chunk(chunk_account: &AccountInfo, signer_account: &AccountInfo) -> ProgramResult {
        let mut chunk_data = ChunkAccount::try_from_slice(&chunk_account.data.borrow())?;
        ...
        if pb.to_bytes().iter().all(|&x| x == 0) {
            ...
            chunk_data.serialize(&mut &mut chunk_account.data.borrow()[..])?;
        }
    ...
    pub fn update_chunk(chunk_account: &AccountInfo, signer_account: &AccountInfo, token: &AccountInfo, data: [u8; 8]) -> ProgramResult {
        let chunk_data = ChunkAccount::try_from_slice(&chunk_account.data.borrow())?;
        let spl_token_account = TokenAccount::unpack(&token.try_borrow_data().unwrap())?;


        if !chunk_data.owner_token.to_bytes().iter().all(|&x| x == 0) && chunk_data.owner_token != *token.key {
            return Err(ProgramError::InvalidInstructionData);
        }

        if spl_token_account.owner != *signer_account.key {
            return Err(ProgramError::IllegalOwner);
        }
    ...
    pub fn update_token(chunk_account: &AccountInfo, signer_account: &AccountInfo, token: Pubkey) -> ProgramResult {
        let chunk_data = ChunkAccount::try_from_slice(&chunk_account.data.borrow())?;
        ...
        msg!("Setting token: {}", token.to_string());

        chunk_data.owner_token = token;
        chunk_data.serialize(&mut &mut chunk_account.data.borrow()[..])?;
